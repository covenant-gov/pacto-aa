...
contract UnitPactoSimple7702Account is Test {
  ...
  function test_Unit_7702Account_OnERC721Received_ReturnsSelector() external view {
  ...
  function test_Unit_7702Account_OnERC1155Received_ReturnsSelector() external view {
    bytes4 _magic = PactoSimple7702Account(payable(_owner)).onERC1155Received(address(0), address(0), 1, 1, '');
    assertEq(_magic, IERC1155Receiver.onERC1155Received.selector);
  }

  function test_Unit_7702Account_OnERC1155BatchReceived_ReturnsSelector() external view {
    ...
    bytes4 _magic =
      PactoSimple7702Account(payable(_owner)).onERC1155BatchReceived(address(0), address(0), _ids, _amounts, '');
    assertEq(_magic, IERC1155Receiver.onERC1155BatchReceived.selector);
  }

  function test_Unit_7702Account_SupportsInterface_ReceiversAndERC165() external view {
    PactoSimple7702Account _account = PactoSimple7702Account(payable(_owner));
    assertTrue(_account.supportsInterface(type(IERC721Receiver).interfaceId));
    assertTrue(_account.supportsInterface(type(IERC1155Receiver).interfaceId));
    assertTrue(_account.supportsInterface(type(IERC165).interfaceId));
    assertFalse(_account.supportsInterface(bytes4(0xffffffff)));
  }
